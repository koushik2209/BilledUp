...
| `db/models.py` | SQLAlchemy models: `Shop` (with `default_pricing` column — "inclusive"/"exclusive"), `Bill` (with `customer_phone`, `pdf_data` LargeBinary, `is_return`, `is_igst`, `subtotal_before_discount`, `bill_discount_type`, `bill_discount_value`, `taxable_amount`, `pricing_type`), `Registration`, `InvoiceSequence`, `PendingBillRecord`, `ProcessedMessage` (dedup), `ShopItemMaster`, `ConversationLog`, `ReportPDF`, `SessionRecord`. |
...
- **GST clarification state (retry-friendly flow)**: When some items fail GST lookup in `_handle_new_bill`, the bill no longer aborts. It enters `awaiting_gst_clarification=True` on `PendingBill`, with resolved items held in `valid_items` and unresolved ones in `failed_items` (full dicts, not just names). `pending.items` stays empty until every item resolves. `_handle_gst_clarification(from_number, message, pending)` re-parses the user's correction, retries `_resolve_gst_for_item` for each new item, merges successes into `valid_items`, and replaces `failed_items` with any remaining failures. When `failed_items` is empty, `valid_items` is promoted to `pending.items` (with return negation applied if needed), the three flags reset, `_assert_bos_invariant` runs, and the bill preview is sent — resuming the normal confirmation flow. `_handle_confirmation` intercepts this state at the top: CANCEL/EDIT still clear the bill; YES is rejected with a nudge; everything else delegates to `_handle_gst_clarification`. `is_return`, `is_bill_of_supply`, `pricing_type`, and discount fields are captured once in `_handle_new_bill` and preserved across all clarification rounds so the final bill matches original intent. Serialization uses `.setdefault` for all three new fields so pre-feature pending rows deserialize safely. This path lives in `services/billing.py` (the legacy handlers still used by tests); the live LLM route in `conversation/executor.py` has not yet been mirrored.
- **Discount-aware parsing and billing**: Claude's `SYSTEM_PROMPT` in `ai/parser.py` follows the STRICT RULES spec — it extracts item-level discounts (`percent`/`flat`), bill-level discounts (`percent`/`flat`/`override`), pricing type (`inclusive`/`exclusive`), and a `needs_confirmation` flag for ambiguous inputs. `ai/sanitizer.validate_parsed_response()` coerces these fields to safe defaults (unknown enum → default, negative numerics → 0). `services.billing._build_pending_from_parser` maps parser output to `PendingBill` using **message-explicit pricing → shop default** precedence. At confirmation, `_compute_bill_from_pending` feeds item and bill discount fields into `calculate_bill`, which applies item discounts in Pass 1, then uses a **scale-ratio approach** for bill-level discounts: compute a single `scale = (subtotal − deduction) / subtotal` (or `target / natural_grand` for override) and multiply each `line_after_item_disc` uniformly, preserving per-item GST rates. Per-line rounding drift is absorbed by nudging the last line (±₹0.10 for flat, ±₹0.50 for override). Override targets above the natural total or equal to zero are clamped AND set `needs_confirmation=True` rather than silently no-op. `BillResult` carries `subtotal_before_bill_discount`, `discount_total`, `taxable_amount`, and `needs_confirmation`. `Bill` table persists `bill_discount_type`, `bill_discount_value`, `subtotal_before_discount`, `taxable_amount`, `pricing_type`. Preview formatter shows a Subtotal / Discount / Taxable block plus a "please double-check" banner when `needs_confirmation` is set. PDF renderer renders the same breakdown for both intra- and inter-state layouts, and credit-note rebuild preserves all new `BillResult` fields.
